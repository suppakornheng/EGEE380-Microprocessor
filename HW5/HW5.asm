;*******************************************
;HW5 press switch random, release to show dice output.asm
;*******************************************
		PROCESSOR PIC16F628
		#include <P16F628.INC>
		...
		movlw	0x00		; w=0
		movwf	TRISB		; TRISB = 0x00 ,Set PORTB as an output port
...
L1:
		btfss	PORTA,1		; Active low, use BTFSS (if active high, use BTFSC)
		goto	button_is_pressed
		goto	button_is_not_pressed

button_is_pressed:
		movf	temp,w		;use [Temp] to call 'DICE_FACES'
		call	DICE_FACES
...
button_is_not_pressed:
		...
		movlw	.7		;
		subwf	temp,w		
		btfss	STATUS,Z	;check if temp=7?
		goto	L1			;No, go back and do it again
	
		movlw	.1
		movwf	temp		;Yes, temp =1		
